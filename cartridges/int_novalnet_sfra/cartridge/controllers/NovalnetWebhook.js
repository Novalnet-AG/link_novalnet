...
function handleInstalment(order, eventData, res, next) {
    if (eventData.transaction.status === 'CONFIRMED' && eventData.instalment.cycles_executed) {
        ...
        var comment = Resource.msgf('novalnet.webhook.instalment.recived', 'novalnet', null, eventData.event.parent_tid, formattedAmount, novalnetHelper.getCurrentDate(), eventTid);
        if (order.custom.novalnetServerResponse) {
            ...
            if (paymentResponse.instalment) {
                ...
                Transaction.wrap(function () {
                    ...
                    paymentResponse.instalment[eventData.instalment.cycles_executed].tid = eventTid;
...
function handleChargeback(order, eventData, res, next) {
    if (eventData.transaction.status === 'CONFIRMED' && eventData.transaction.amount) {
        var formattedAmount = novalnetHelper.getFormattedAmount(eventData.transaction.amount, eventData.transaction.currency);
        var comment = Resource.msgf('novalnet.webhook.chargeback.executed', 'novalnet', null, eventData.event.parent_tid, formattedAmount, novalnetHelper.getCurrentDate(), eventData.event.tid);
        updateTransactionDetails(order, comment);
        sendWebHookMail(res, next, eventData.transaction.order_no, comment);
        displayMessage(res, next, comment);
    }
...
function handleCredit(order, eventData, res, next) {
    ...
    var comment = '\n' + Resource.msgf('novalnet.webhook.credit.executed', 'novalnet', null, eventData.event.parent_tid, formattedAmount, novalnetHelper.getCurrentDate(), eventData.event.tid);
...
function handleTransactionUpdate(order, eventData, res, next) {
    ...
    if (['PENDING', 'ON_HOLD', 'CONFIRMED', 'DEACTIVATED'].indexOf(status) > -1) {
        ...
        if (status === 'DEACTIVATED') {
            ...
            comment = '\n' + Resource.msgf('novalnet.webhook.transaction.changed', 'novalnet', null, eventData.event.tid, novalnetHelper.getCurrentDate());
            ...
            comment = '\n' + Resource.msgf('novalnet.webhook.transaction.updated', 'novalnet', null, eventData.event.tid, formattedAmount, novalnetHelper.getCurrentDate());
...
function handleTransactionRefund(order, eventData, res, next) {
    if (eventData.transaction.refund.amount) {
        ...
        var comment = Resource.msgf('novalnet.webhook.transaction.refunded', 'novalnet', null, eventData.event.parent_tid, formattedAmount);

        if (eventData.transaction.refund.tid) {
            comment += Resource.msgf('novalnet.webhook.transaction.refunded', 'novalnet', null, eventData.transaction.refund.tid, formattedAmount);
        }
        updateTransactionDetails(order, comment);
        sendWebHookMail(res, next, eventData.transaction.order_no, comment);
        displayMessage(res, next, comment);
...
function handleTransactionCaptureCancel(order, eventData, res, next) {
    ...
    if (eventData.event.type === 'TRANSACTION_CAPTURE') {
        ...
        if (['INSTALMENT_INVOICE', 'INSTALMENT_DIRECT_DEBIT_SEPA'].indexOf(eventData.transaction.payment_type) > -1) {
            ...
            var transactionComment = novalnetHelper.formComments(eventData, true);
            novalnetHelper.addOrderNote(order, transactionComment, false);
        } else if (eventData.transaction.payment_data) {
            if (eventData.transaction.payment_data.paypal_account) {
                var paymentResponse = JSON.parse(order.custom.novalnetServerResponse);
                paymentResponse.transaction.payment_data.paypal_account = eventData.transaction.payment_data.paypal_account;
                paymentResponse.transaction.payment_data.paypal_transaction_id = eventData.transaction.payment_data.paypal_transaction_id;
                Transaction.begin();
                Transaction.wrap(function () {
                    order.custom.novalnetServerResponse = JSON.stringify(paymentResponse);
                });
...
function handleCommunicationFailure(order, eventData) {
    ...
    try {
        if (status === 'SUCCESS' || ['FAILURE', 'DEACTIVATED'].indexOf(eventData.transaction.status) > -1) {
            return novalnetHelper.handleFailure(order, eventData);
        }
        return novalnetHelper.handleSuccess(order, eventData);
    } catch (e) {
        return e.message;
    }
}
...
function validateChecksum(eventData) {
    ...
    if (eventData.transaction.amount) {
