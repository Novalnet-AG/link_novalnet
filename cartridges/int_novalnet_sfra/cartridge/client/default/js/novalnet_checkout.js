...
$(document).ready(function () {
    ...
    */
    ...
    $('#novalnet_sepa_iban, #novalnet_guaranteed_sepa_iban, #novalnet_instalment_sepa_iban').on('input change', function () {
		var paymentType = this.id;
		var paymentType = paymentType.replace('_iban', '');
		if(this.value) {
			displayBicField(this.value, paymentType);
		}
	});
	
	function displayBicField(iban, paymentType) {
		var countryCode = iban.substring( 0, 2 );
		if(['CH', 'MC', 'SM', 'GB'].indexOf(countryCode.toUpperCase()) > -1) {
			$('#' + paymentType + '_bic_field').show();
		}
		else {
			$('#' + paymentType + '_bic_field').hide();
		}
	}
    ...
    document.querySelector('button.submit-payment').addEventListener('click', function (event) {
        ...
        if (selectedPaymentMethod) {
            ...
            if (['NOVALNET_GUARANTEED_INVOICE', 'NOVALNET_GUARANTEED_SEPA', 'NOVALNET_INSTALMENT_SEPA', 'NOVALNET_INSTALMENT_INVOICE', 'NOVALNET_SEPA'].indexOf(selectedPaymentMethod) > -1) {
                if (['NOVALNET_GUARANTEED_SEPA', 'NOVALNET_INSTALMENT_SEPA', 'NOVALNET_SEPA'].indexOf(selectedPaymentMethod) > -1 && $('#novalnet_sepa_payment_form, #novalnet_guaranteed_sepa_payment_form, #novalnet_instalment_sepa_payment_form').is(':visible')) {
                    if ($('#' + paymentType + '_iban') !== undefined) {
                        var iban = NovalnetUtility.formatAlphaNumeric($('#' + paymentType + '_iban').val());
                        if (iban === '' || ($('#' + paymentType + '_bic').is(':visible') && $('#' + paymentType + '_bic').val() == '')) {
                            showErrorMsg($('#novalnet_invalid_account_details_lang').val());
                ...
                if ($('#' + paymentType + '_dob') !== undefined && $('#' + paymentType + '_dob').val() !== undefined) {
                    if ($('#' + paymentType + '_dob').val() === '' || !NovalnetUtility.validateDateFormat($('#' + paymentType + '_dob').val())) {
                        showErrorMsg($('#novalnet_invalid_dob_lang').val());
                        event.preventDefault();
                        event.stopImmediatePropagation();
                    }
                }
