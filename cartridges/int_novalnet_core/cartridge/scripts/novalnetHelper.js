...
function getTransactionDetails(order, paymentInstrument) {
    ...
    if (['INVOICE', 'PREPAYMENT', 'DIRECT_DEBIT_SEPA', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_DIRECT_DEBIT_SEPA', 'CASHPAYMENT'].indexOf(paymentType) > -1) {
        if (config.dueDate) {
            var date = new Date();
    ...
    if (['DIRECT_DEBIT_SEPA', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_DIRECT_DEBIT_SEPA', 'CREDITCARD', 'PAYPAL'].indexOf(paymentType) > -1) {
...
function getPaymentComments(paymentResponse) {
    ...
    if (['GUARANTEED_INVOICE', 'INSTALMENT_INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_DIRECT_DEBIT_SEPA'].indexOf(paymentType) > -1 && status === 'PENDING') {
        ...
        if (paymentType === 'INSTALMENT_INVOICE') {
            ...
            comment += Resource.msg('novalnet.invoice.paymentreference', 'novalnet', null) + ' 1: ' + paymentResponse.transaction.invoice_ref + newLine;
            comment += Resource.msg('novalnet.invoice.paymentreference', 'novalnet', null) + ' 2: ' + tid + newLine;
        }
        ...
        while (systemOrders.hasNext()) {
            ...
            if (!empty(order.custom.novalnetServerResponse)) {
                ...
                if (paymentType === 'CREDITCARD') {
                    ...
                    labelText = 'ending in ' + paymentData.card_number.substring(paymentData.card_number.length - 4, paymentData.card_number.length);
                    labelText += '  expires( ';
                    labelText += (paymentData.card_expiry_month.toString().length > 1) ? paymentData.card_expiry_month : '0' + paymentData.card_expiry_month;
                    labelText += '/' + expiryYear.substring(expiryYear.length - 2, expiryYear.length) + ' )';
                } else if (['DIRECT_DEBIT_SEPA', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_DIRECT_DEBIT_SEPA'].indexOf(paymentType) > -1) {
                    labelText = 'IBAN ' + paymentData.iban;
                } else {
                    labelText = 'PayPal account ' + paymentData.paypal_account;
                    delete paymentData.paypal_transaction_id;
                }
...
exports.updateInstalmentDetails = function (order, response) {
    if (response.instalment) {
        ...
        instalmentDetails[response.instalment.cycles_executed].tid = response.event.tid;
