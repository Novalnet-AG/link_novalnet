        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('.novalnet_show_instalment_refund').on('click', function () {
                    ...
                    if($('#novalnet_refund_amount_' + tableId).val()) {
						var openAmount = $('#novalnet_refund_amount_' + tableId).val();
						$('#novalnet_refund_amount_' + tableId).val(parseInt(openAmount));
					}
                    $(this).hide();
                });
                ...
                $('#novalnet_cancel_all_instalment_cycle, #novalnet_cancel_remaining_instalment_cycle').on('click', function () {
                    var tid = $('#novalnet_transaction_id').val();
                    var orderNo = $('#novalnet_order_no').val();
                    var orderToken = $('#novalnet_order_token').val();
                    
                    var cancel_type =  'cancel_remaining_cycles';
                    var confirmMsg = novalnetAdmin.resources.cancelRemainingCycleConfirmMsg;
                    if($(this).attr('id') == 'novalnet_cancel_all_instalment_cycle') {
						cancel_type =  'cancel_all_cycles';
						confirmMsg = novalnetAdmin.resources.cancelAllCycleConfirmMsg;
					}
                    ...
                    function callbackFunction(btn) {
                        if (btn === 'yes') {
                            var data = { tid: tid, orderNo: orderNo, orderToken: orderToken, action: 'instalment_cancel',cancelType: cancel_type };
                            executeAction(data);
                        }
                    }
                });
                ...
                $('.novalnet_submit_refund_form').on('click', function () {
                    ...
                    function callbackFunction(btn) {
                        if (btn === 'yes') {
                            var data = {
                                tid: tid, orderNo: orderNo, orderToken: orderToken, refundAmount: refundAmount, reason: refundReason, action: 'refund', instalment_cycle: id
                            };
                            executeAction(data);
                        }
                    }
