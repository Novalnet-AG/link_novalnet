...
function orderTransaction() {
	...
	var paymentInstrument = novalnetHelper.getPaymentInstrument(order, novalnetPaymentMethod);
	var orderAmount = Math.floor(paymentInstrument.paymentTransaction.amount.value * 100).toString();
...
function executeAmountBooking(params, order) {
	var paymentInstrument = novalnetHelper.getPaymentInstrument(order, order.custom.novalnetPaymentMethod);
    ...
    data.transaction = {
        payment_type: order.custom.novalnetPaymentMethod,
        amount: params.amount.stringValue,
...
function executeVoidCapture(params, order) {
    ...
    if (response.result.status_code === 100) {
        ...
        if (action === 'instalment_cancel') {
            comment = '\n' + Resource.msgf('novalnet.instalment.cancel', 'novalnet', null, params.tid.stringValue);
            status = 'DEACTIVATED';
        } else {
